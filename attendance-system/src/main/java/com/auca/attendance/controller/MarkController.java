...
public class MarkController {
    ...
    public ResponseEntity<ApiResponse<List<MarkColumn>>> getColumns(@PathVariable Long moduleId) {
        return ResponseEntity.ok(ApiResponse.success(markService.getColumns(moduleId)));
    }

    @PostMapping("/modules/{moduleId}/columns")
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<ApiResponse<MarkColumn>> createColumn(
            @PathVariable Long moduleId,
            @Valid @RequestBody MarkColumnRequest request,
            @AuthenticationPrincipal User currentUser) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("Column created", markService.createColumn(moduleId, request, currentUser)));
    }
    ...
    @PreAuthorize("hasAnyRole('INSTRUCTOR','ADMIN')")
    public ResponseEntity<ApiResponse<List<MarkEntry>>> getMarks(@PathVariable Long moduleId) {
        return ResponseEntity.ok(ApiResponse.success(markService.getMarks(moduleId)));
    }

    @PostMapping("/modules/{moduleId}/marks")
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<ApiResponse<List<MarkEntry>>> submitMarks(
            @PathVariable Long moduleId,
    ...
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<ApiResponse<MarkEntry>> updateMark(
